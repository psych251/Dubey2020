...
clear all; 
all_data = loadjson('../data/curios_rep_explorative_data_without_repetition.json'); %read data in a big cell here
...
for cond = 1:2 %conditions 1-2 (if you want to *plot* results from Phase 1, then you don't need the for loop and you will have to call the conditions individually as the plotting file only plots the condition separately)
    ...
    n = length(all_data); %number of subjects
    ...
    for i = 1:n %number of subjects
        ...
        for f = 1:height(field_names)
            ...
            for j = 1:length(subject_data) %first get condition of that subject
                if(strcmp(subject_data{1,j}.trial_type,'survey-multi-choice')==1)
                    c = j; %first get index of the quiz answer (get the last response of the quiz, subjects could take quiz multiple times)
                end
            end
            if(strfind(subject_data{1,c}.responses, 'CORRECTNESS') > 0)
                condition = 1; %condition 1 if correct response had more in it
            elseif(strfind(subject_data{1,c}.responses, 'RANDOMLY') > 0)
        ...
        if(condition == cond) %only analyze for that condition
            ...
            answer_reveal = zeros(1,20); %store whether subject revealed answer or not
        
            for j = 4:2:42 %now extract responses for all trivia questions in phase 1
                a = subject_data{1,j}.responses; 
                a = extractNum(a);
            ...
            for j = 1:20
                if(subject_data{1,counter}.key_press == 65) %if subject choose to reveal the answer
                    answer_reveal(j)=1;
